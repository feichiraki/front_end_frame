...
发行uniapp微信小程序报错[ app.json 文件内容错误] app.json: 未找到 ["sitemapLocation"] 对应的 sitemap.json 文件

**解决：重新打开微信小程序即可。**

一开始项目文件中确实没有sitemap.json文件，重新打开此项目后就有了这个文件。
		但是仍然报错，说找不到这个文件。
		此时我们回到我们使用的编辑器，这里以Hbuilder X为例。

> ①点击菜单栏运行--运行到小程序模拟器--停止微信开发者工具
>
> ②之后点击运行--运行到小程序模拟器--微信开发者工具重新运行。
>
> ③回到微信开发者工具可以看见问题解决了。
...
原来是接口和微信公众平台域名不对应，微信公众平台缺没有更改。登录微信公众平台，选择 开发->开发管理->服务器域名->request合法域名 将它更改就可以了！

> 但是这里并没有解决，前面微信开发者工具调试没有问题了，但是体验版扫码还是出问题。



##### 出错原因

将***\*不校验合法域名\****取消打勾，之后重新上传代码，但是发现还是不可以，经过尝试发现是微信缓存的问题，将**微信的缓存清掉**，将**小程序使用记录删除**之后重新扫码打开就正常访问了。

> 　设置 — 通用 — 存储空间

<img src="遇到的问题.assets/image-20240325173659849.png" alt="image-20240325173659849" style="zoom:67%;" />